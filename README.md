...
The implementation reconstructs cash flows from the supplied profiles,
events, rates, messages/images metadata, and payment options, then runs a
deterministic 90-day safety forecast. It writes the required `output.csv` at
the repository root. Validate the generated file with:
